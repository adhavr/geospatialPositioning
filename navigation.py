...
for pair in avgpairs:
    for i in range(0,len(inpic)-1):
        for j in range(0,len(inpic)-1):
            if inpic[i][0] in pair and inpic[j][0] in pair and inpic[i][3] == inpic[j][3] and inpic[i][0] != inpic[j][0]:
                inpic[i][0] = pair[0]
...
print("\n\n\n", inpic)
print(finalcoords)
print("(" + str(totx/len(finalcoords)) + ", " + str(toty/len(finalcoords)) + ")")